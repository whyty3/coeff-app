...
import { Activity, Shield, AlertTriangle, Share2, Download, Mail, ArrowRight, Layers, Lock, RefreshCw, Plus, Trash2, CheckCircle, XCircle, Wallet, FileText, Database } from 'lucide-react';

/**
 * coeff.io - Portfolio Risk & Correlation Analyzer
 * VERSION: Production v2.11 (Final Release Candidate)
 */
...
// --- BRAND ASSET: THE REGRESSION MARK (HIGH CONTRAST) ---
const CoeffLogo = ({ className = "w-6 h-6" }) => (
  <svg 
    ...
    className={className}
  >
    {/* Abstract Hexagon Container - Slate Blue for structure */}
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      responses.forEach((res, i) => {
          ...
          let hist = [];
          if (res && res._warning) isUsingSynthetic = true;
          ...
          const latestPoint = hist[0]; 
          fetchedPrices[ticker] = latestPoint.close;
      ...
      // --- 4. FRAGILITY SCORE (WEIGHTED + NON-LINEAR) ---
      let weightedSum = 0;
      let weightsSum = 0;
      let count = 0;
      ...
      // Non-Linear Scaling: Sqrt(Corr) * 100
    ...
    <div className="min-h-screen bg-slate-950 text-slate-400 font-sans selection:bg-indigo-500/30">
        ...
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="hidden sm:flex items-center gap-3">
             {!PROXY_URL && (
                 <input 
                   type="password" 
                   placeholder="Dev Mode: API Key"
                   value={apiKey}
                   onChange={(e) => saveKey(e.target.value)}
                   className="bg-slate-950 border border-slate-800 rounded px-3 py-1.5 text-xs w-32 focus:w-48 transition-all outline-none"
                 />
             )}
             
             {PROXY_URL && (
                 <div className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-900/20 border border-indigo-900/50 rounded">
             ...
             {walletAddress ? (
                 <div className="flex items-center gap-2 bg-slate-900 border border-slate-700 text-slate-300 px-4 py-1.5 rounded-full text-xs font-mono">
                    <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div>
                    {formatAddress(walletAddress)}
                 </div>
             ) : (
                 <button onClick={connectWallet} className="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-1.5 rounded text-sm font-medium transition-colors flex items-center gap-2">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        <div className="text-center mb-12">
          ...
          {dataQualityMsg && (
              <div className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-900/20 border border-amber-900/50 text-amber-400 text-xs font-bold animate-in fade-in slide-in-from-top-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          <Card className="space-y-4">
            <div className="flex justify-between items-center mb-2">
              ...
              <button onClick={addAsset} className="text-indigo-400 hover:text-indigo-300">
            ...
            <div className="space-y-3">
              {assets.map((asset, i) => (
                <div key={i} className="flex gap-2 items-center">
                  <input value={asset.ticker} onChange={(e) => updateAsset(i, 'ticker', e.target.value)} placeholder="Ticker" className="flex-1 bg-slate-900 border border-slate-800 rounded px-3 py-2 text-white text-sm font-mono uppercase focus:border-indigo-500 outline-none" />
                  <div className="w-24 bg-slate-950 border border-slate-800 rounded px-3 py-2 text-xs font-mono text-right flex items-center justify-end">
                      {latestPrices[asset.ticker] ? <span className="text-emerald-400 font-bold">${latestPrices[asset.ticker].toFixed(2)}</span> : <span className="text-slate-600">-</span>}
                  </div>
                  <input type="number" value={asset.weight} onChange={(e) => updateAsset(i, 'weight', e.target.value)} placeholder="%" className="w-16 bg-slate-900 border border-slate-800 rounded px-3 py-2 text-white text-sm font-mono text-right focus:border-indigo-500 outline-none" />
                  <button onClick={() => removeAsset(i)} className="text-slate-600 hover:text-rose-400 px-1"><Trash2 className="w-4 h-4" /></button>
                </div>
              ))}
            </div>
            ...
            <div className="pt-4 border-t border-slate-800">
              ...
              <button onClick={runAnalysis} disabled={isProcessing} className="w-full bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 text-white font-bold py-3 rounded-lg shadow-lg shadow-indigo-500/20 transition-all flex items-center justify-center gap-2">
            ...
            {!results ? (
              ...
              <div className="w-full h-full flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <div className="flex-1">
                  ...
                  <div className="grid" style={{ gridTemplateColumns: `repeat(${assets.length}, 1fr)` }}>
                    {results.matrix.map((row, i) => (
                      row.map((val, j) => (
                        <div key={`${i}-${j}`} className="aspect-square flex items-center justify-center text-[10px] font-mono border border-slate-900/50 relative group" style={{ backgroundColor: getHeatmapColor(val) }}>
                          <span className="relative z-10 text-white drop-shadow-md opacity-80 group-hover:opacity-100">{val.toFixed(2)}</span>
                  ...
                  <div className="grid mt-1" style={{ gridTemplateColumns: `repeat(${assets.length}, 1fr)` }}>
                    {assets.map(a => (
                      <div key={a.ticker} className="text-center text-[9px] text-slate-500 font-mono">{a.ticker}</div>
                    ))}
                  </div>
                </div>
              </div>
