...
import { Activity, Shield, AlertTriangle, Share2, Download, Mail, ArrowRight, Layers, Lock, RefreshCw, Plus, Trash2, CheckCircle, XCircle, Wallet, FileText, Database, Import, BookOpen, FileDown, Smartphone, X } from 'lucide-react';

/**
 * coeff.io - Portfolio Risk & Correlation Analyzer
 * VERSION: Production v3.17 (Mobile Optimization: Better Export UX)
 */
...
export default function CoeffRiskAnalyzer() {
  ...
  const handleExportText = () => {
    ...
    report += `Fragility Score: ${results.fragilityScore}/100\nBenchmark Beta: ${results.beta} (vs ${benchmark})\nAvg Correlation: ${results.avgCorr}\n\nPortfolio Composition:\n`;
    assets.forEach(a => {
       const priceInfo = latestPrices[a.ticker] ? `${getCurrencySymbol(latestPrices[a.ticker].currency)}${latestPrices[a.ticker].price}` : 'N/A';
    ...
    a.href = url; a.download = `risk_report_${Date.now()}.txt`;
      ...
      <nav className="border-b border-slate-800 bg-slate-900/50 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
             ...
             {walletAddress ? (
                 <div className="hidden sm:flex items-center gap-2 bg-slate-900 border border-slate-700 text-slate-300 px-3 py-1.5 rounded-full text-xs font-mono">
